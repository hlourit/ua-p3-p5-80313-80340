...
public class ex71 {
    ...
    public static void main(String args[]) throws IOException {
        Sigla sl = Sigla.parseFile(new File("companhias.txt"));
        ArrayList<Voo> voos = Voo.parseFile(new File("voos.txt"), sl);
    ...
    public static Companhia[] printTabelaToTxtFile(ArrayList<Voo> voos) throws FileNotFoundException {
        ...
        PrintWriter pwr = new PrintWriter(new File("Infopublico.txt"));
    ...
    public static Companhia[] printTabelaToBinFile(ArrayList<Voo> voos) throws FileNotFoundException {
        HashMap<String,Companhia> tmp=new HashMap<>();
        PrintWriter pwr = new PrintWriter(new File("Infopublico.bin"));
        pwr.println("Hora    Voo     Companhia           Origem                      Atraso    Obs");
        for (Voo voo : voos) {
            pwr.printf("%02d:%02d   %-8s%-20s%-29s", voo.time() / 60, voo.time() % 60, voo.id(), voo.cmp(), voo.orig());
            if(voo.t_atr()!=0){
    ...
    public static void fileOrigens(Origem[] infoOrigem) throws FileNotFoundException{
        sortVoos(infoOrigem);
        PrintWriter pwr=new PrintWriter(new File("cidades.txt"));
